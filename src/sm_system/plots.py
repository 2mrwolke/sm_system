...
try:
    ...
    raise ImportError(
        "Plotting requires matplotlib. Install with `pip install sm-system[plot]`."
    ) from e
...
def plot_bode_measurement(
    ...
    ax[0].semilogx(
        freq_gt, mag_gt, color=colors[0], label=plot_labels[0][0]
    )  # Bode magnitude plot
    if freq_mag is not None and mag is not None:
        # Scatter plot of magnitude measurements
        ax[0].scatter(freq_mag, mag, color=colors[1], label=plot_labels[0][1])
    ...
    ax[0].axvline(
        x=sampling_rate / 2, color="grey", linestyle="-"
    )  # , label='Nyquist freq')
    ax[0].text(
        sampling_rate // 2,
        limits[0][0],
        f"{sampling_rate / 2000:.0f} kHz    ",
        rotation=-90,
    ...
    ax[1].semilogx(
        freq_gt, phase_gt, color=colors[2], label=plot_labels[1][0]
    )  # Bode phase plot
    if freq_phase is not None and phase is not None:
        # Scatter plot of phase measurements
        ax[1].scatter(
            freq_phase, phase, color=colors[3], label=plot_labels[1][1]
        )
    ...
    if save and save_individual:
        for i, subplot_title in enumerate(
            ["Magnitude - " + plot_title, "Phase - " + plot_title]
        ):
            individual_fig, individual_ax = plt.subplots(figsize=(10, 6))
            if i == 0:
                individual_ax.semilogx(
                    freq_gt, mag_gt, color=colors[0], label=plot_labels[i][0]
                )
                if freq_mag is not None and mag is not None:
                    individual_ax.scatter(
                        freq_mag, mag, color=colors[1], label=plot_labels[i][1]
                    )
                individual_ax.set_ylabel("Magnitude [dB]", fontsize=20)
            else:
                individual_ax.semilogx(
                    freq_gt, phase_gt, color=colors[2], label=plot_labels[i][0]
                )
                if freq_phase is not None and phase is not None:
                    individual_ax.scatter(
                        freq_phase,
            ...
            individual_ax.grid(
                which="both", axis="both", linestyle="--", linewidth=0.5
            )
            ...
            individual_fig.savefig(
                f"{path}{plot_title}_{subplot_title.split(' ')[0]}.png",
                dpi=600,
                bbox_inches="tight",
            )
