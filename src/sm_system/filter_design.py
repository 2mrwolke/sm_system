...
def compute_minimum_phase(magnitude_spectrum, dB=True, degree=True):
    ...
    magnitude_spectrum = np.hstack(
        [magnitude_spectrum, np.flip(magnitude_spectrum[1:])]
    )
    ...
    # Compute the imaginary part of the Fourier transform of the adjusted
    # cepstrum
...
def FIR_Filter_Freq_Sampling(
    freq_hz, magnitude_db, phase_deg, tabs, samplerate, weights=None
):
    ...
    if weights is None:
        ...
        fir_truncated = least_squares(
            A=freq_matrix, b=desired_freq_resp, w=weights
        ).real
